...
async function runSpec(spec) {
  ...
  const pid = url.get('PROLIFIC_PID') || '';
  ...
  async function finish() {
    root.innerHTML = '<div class="card"><p>Saving your responses&hellip;</p></div>';
    const payload = {
      study: spec.study,
      prolific_pid: pid,
    ...
    let ok = false;
    // In production the data sink is proliferate (proliferate.alps.science), which
    // stores the responses and performs the Prolific completion redirect itself.
    // With no proliferate library loaded we fall back to the local serve.py endpoint,
    // which is what replay_check.py and local review use.
    if (window.proliferate && typeof window.proliferate.submit === 'function') {
      try { await window.proliferate.submit(payload); ok = true; }
      catch (e) { ok = false; }
    }
    for (let attempt = 0; attempt < 3 && !ok && !window.proliferate; attempt++) {
      try {
