 ...
 *   {"type":"page","shuffle":bool,"items":[stimuli|question...]}
 *       // one screen holding several items, as a vignette shown with its rating or a
 *       // matrix; `shuffle` reorders the questions and leaves stimuli where they are
 * Any stimuli, question or page may also carry
 *   "show_if": {"variable":..,"op":">"|">="|"<"|"<="|"=="|"!="|"in"|"not_in","value":..}
 *       // shown only when that earlier answer satisfies the test; an unanswered
 *       // variable never does. Hidden items are skipped both ways and not submitted.
 *   "pipe": {"<slot label>": "<variable-name>"}
 *       // every occurrence of the label in the text is replaced by that earlier
 *       // answer, as the reconstruction's slot labels stand for the participant's
 *       // own entries; a blank answer leaves the label in place
 */
...
function expandRuns(blocks, rng) {
  ...
  for (const b of blocks) {
    if (b.type === 'group') {
      ...
      let items = b.items.slice();
      if (b.shuffle) {
        const qs = shuffled(items.filter(x => x.type !== 'stimuli'), rng);
        items = items.map(x => (x.type === 'stimuli' ? x : qs.shift()));
      }
      runs.push([Object.assign({}, b, { items })]);
...
function widget(item) {
  ...
  if (rc.type === 'free-response') {
    ...
    const max = limit ? ` maxlength="${limit}"` : '';
    return {
      html: rc['single-line']
        ? `<input type="text" id="${id}" class="freetext line" autocomplete="off"${max}>`
        : `<textarea id="${id}" rows="5" class="freetext"${max}></textarea>`,
...
async function runSpec(spec) {
  ...
  let current = null;          // [{item, w}] for the entries on screen
  let skipArmed = null;        // page index whose skip has been offered once
  ...
  function shown(item) {
    const c = item.show_if;
    if (!c) return true;
    const got = answers[c.variable];
    const v = got ? got.answer : undefined;
    if (v === undefined || v === null || v === '') return false;
    return OPS[c.op](v, c.value);
  }
  // A page is on screen only if it, and at least one of its entries, is shown.
  const entriesOf = page => (page.type === 'page' ? page.items : [page]).filter(shown);
  const pageShown = page => shown(page) && entriesOf(page).length > 0;

  function piped(text, item) {
    let out = String(text == null ? '' : text);
    for (const [label, name] of Object.entries(item.pipe || {})) {
      const got = answers[name];
      const v = got && typeof got.answer === 'string' ? got.answer.trim() : '';
      if (v) out = out.split(label).join(esc(v));
    }
    return out;
  }
  ...
  function commit() {
    ...
    const blanks = current.filter(({ w }) => {
      if (!w || w.optional) return false;
      const v = w.read();
    ...
    if (blanks.length && skipArmed !== i) {
      skipArmed = i;
      const err = document.getElementById('err');
      if (err) err.textContent = (current.length > 1
        ? 'Some questions on this page are unanswered. You may leave them blank if '
        : 'You may leave this question blank if ')
        + 'you would rather not answer — press Continue again to skip.';
      return false;
    }
    skipArmed = null;
    for (const { item, w } of current) {
      if (!w) continue;
      const v = w.read();
  ...
  // Move to the next page that is shown, in either direction.
  function step(dir) {
    do { i += dir; } while (i > 0 && i < pages.length && !pageShown(pages[i]));
  }
  ...
  function render() {
    ...
    for (const it of entries) {
      if (order.indexOf(it['variable-name']) === -1) order.push(it['variable-name']);
    }
    ...
    let p = i - 1;
    while (p > 0 && !pageShown(pages[p])) p--;
    const prev = pages[p];
    const canBack = i > 0 && !(prev && prev.type === 'timed-ideas'
                               && timedDone[prev['variable-name']]);
    const pct = Math.round(100 * i / pages.length);

    if (page.type === 'timed-ideas') return renderTimed(page, canBack, pct);

    let body = '';
    current = [];
    for (const item of entries) {
      if (item.type === 'stimuli') {
        body += `<div class="stim">${piped(item.content, item)}</div>`;
        current.push({ item, w: null });
      } else {
        const w = widget(item);
        body += `<div class="q"><div class="qtext">${piped(item.question, item)}</div>` +
          `${w.html}</div>`;
        current.push({ item, w });
      }
    }
    let next = i + 1;
    while (next < pages.length && !pageShown(pages[next])) next++;
    root.innerHTML =
      ...
      `<button id="next" class="primary">${
        page.consent ? 'I consent, begin the study'
          : next >= pages.length ? 'Finish' : 'Continue'}</button>` +
      `</div></div>`;
    let focused = false;
    for (const { item, w } of current) {
      if (!w) continue;
      ...
      if (w.focus && !focused) { w.focus(); focused = true; }
    }
    const back = document.getElementById('back');
    if (back) back.onclick = () => { current = null; step(-1); render(); };
  ...
  async function finish() {
    ...
    // Shaped for proliferate's CSV export: it writes one CSV per TOP-LEVEL key, one
    // row per element for a list and one row per participant for an object. So the
    // responses go in a `trials` list (-> one clean row per item) and everything
    // else in a flat `subject_information` object (-> one row per participant). It
    // also produces -merged.csv only when exactly one top-level list is present.
    // Nesting the answers under one key, as an earlier version did, put a stringified
    // object in every cell and made -merged.csv fail on the server.
    // An answer whose item is hidden now (they went back and changed the answer it
    // depends on) was never part of the path they finished on.
    const hidden = new Set();
    for (const page of pages) {
      for (const it of page.type === 'page' ? page.items : [page]) {
        if (!shown(page) || !shown(it)) hidden.add(it['variable-name']);
      }
    }
    const trials = order
      .filter(name => answers[name] !== undefined && !hidden.has(name))
      .map((name, k) => ({
        position: k + 1,
        variable_name: name,
        answer: answers[name].answer,
        condition: answers[name].condition,
      }));
    const payload = {
      trials: trials,
      subject_information: {
        ...
        prolific_pid: url.get('PROLIFIC_PID'),
